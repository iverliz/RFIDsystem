...
class StudentRecord(tk.Frame):
    ...
    def search_student(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT Student_id, Student_name, grade_lvl FROM student WHERE Student_name LIKE %s or Student_id LIKE %s",
            (f"%{keyword}%", f"%{keyword}%"))   
        rows = cursor.fetchall()
        for row in rows:
            self.student_table.insert("", "end", values=row)
    ...
    def sort_table ():
        data = [()self.student_table.set(k, col) for k in self.student_table.get_children("")]
        data.sort (reverse=reverse)
        for index, (_, k) in enumerate(data):
            self.student_table.move(k, "", index)
        self.student_table.heading(col, command=lambda: self.sort_table(col, not reverse))