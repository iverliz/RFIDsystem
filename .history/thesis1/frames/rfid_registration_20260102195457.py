...
import sys, os
import serial, threading, time   # ✅ ADDED
...
class RfidRegistration(tk.Frame):
    ...
    def read_serial(self):
        while self.ser and self.ser.is_open:
            ...
            if uid:
                self.root.after(0, self.rfid_var.set, uid)
    ...
    def add_record(self):
        ...
        if not uid:
            messagebox.showwarning("Missing", "Scan RFID first")
            return

        if self.check_duplicate_rfid(uid):
            messagebox.showerror("Duplicate", "RFID already exists")
            return
        ...
        try:
            ...
            """, (
                uid, self.name_var.get(), self.address_var.get(),
