...
class MainDashboard(tk.Frame):
    def __init__(self, parent):
        ...
        self.parent = parent  # reference to Rfid app which has all frames
    ...
    # -----------------------
    # Function to open a frame
    # -----------------------
    def open_frame(self, frame_class):
        """
        Raises the frame in the main area.
        """
        frame_name = frame_class.__name__
        if frame_name in self.parent.frames:
    ...
    def create_menu_button(self, text, frame_class):
        btn = tk.Button(
            ...
            pady=15,
            font=("Arial", 12, "bold"),
            command=lambda: self.open_frame(frame_class)  # <-- call the function
        )
