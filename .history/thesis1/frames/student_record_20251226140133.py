...
class Student(tk.Frame):
    ...
    def upload_photo(self):
        ...
        def load_data(self):
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT student, Student_name, Student_id, grade_lvl,
                           Guardian _name, Teacher_name, grade, Guardian_contact
                    FROM rfid_system
                """)
        ...
        def add_student(self):
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO  student (Student_name, Student_id, grade_lvl, Guardian_name, Guardian_contact, Teacher_name)"""
                )
                ...
                if not self.grade_var.get():
                    messagebox.showerror("Error", "Please enter a student grade.")
                    return

                if not self.guardian_name_var.get():
                    messagebox.showerror("Error", "Please enter a guardian name.")
                    return

                if not self.guardian_contact_var.get():
                    messagebox.showerror("Error", "Please enter a guardian contact.")
                    return
                    
                elif not self.guardian_contact_var.get():
                    messagebox.showerror("Error", "Please enter a number not a letter or string .")
                    return

                if not self.teacher_name_var.get():
                    messagebox.showerror("Error", "Please enter a teacher name.")
                    return
    ...
    def delete_student(self):
        ...
        try:
            conn = db_connect()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM student WHERE student_id=%s", (self.student_id_var.get(),))
            conn.commit()
            self.load_data()
            self.clear_fields()
        finally:
