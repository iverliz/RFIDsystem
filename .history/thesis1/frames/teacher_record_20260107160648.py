...
class TeacherRecord(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Button(
            ...
            font=("Arial", 10, "bold"), command=self.add_teacher
        ).grid(row=0, column=0, padx=5)

        tk.Button(
            btn_frame, text="EDIT", width=10, bg="#2196F3", fg="white",
            font=("Arial", 10, "bold"), command=self.edit_teacher
        ).grid(row=0, column=1, padx=5)

        tk.Button(
            btn_frame, text="DELETE", width=10, bg="#F44336", fg="white",
            font=("Arial", 10, "bold"), command=self.delete_teacher
    ...
    def search_teacher(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT teacher_name, teacher_grade, photo_path FROM teacher WHERE teacher_name LIKE %s",
            (f"%{keyword}%",)
        )
        rows = cursor.fetchall()
