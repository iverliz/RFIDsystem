...
class RfidRegistration(tk.Frame):
    def __init__(self, root):
        ...
        self.image_label = tk.Label(self.fetcher_frame, bg="white")
        self.image_label.pack(pady=5)

        tk.button = tk.Button(self.fetcher_frame, text="Upload Image", command=self.upload_image)
        tk.button.pack(pady=5)
    ...
    def upload_image(self):
        ...
        if file_path:
            self.image_label.config(image=self.load_image(file_path))
            self.image_label.image = self.load_image(file_path)

            self.img = Image.open(file_path)
            self.img = self.img.resize((120, 120))
