...
class FetcherRecord(tk.Frame):
    ...
    def load_data(self):
        ...
        try:
            conn = db_connect()
            cursor = conn.cursor()
            cursor.execute("SELECT fetcher_id, Fetcher_name, Address, contact, photo_path FROM fetcher")
    ...
    def delete_fetcher(self):
        ...
        try:
            conn = db_connect()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM fetcher WHERE fetcher_id=%s", (fetcher_id,))
            conn.commit()
    ...
    def search_fetcher(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT fetcher_id, Fetcher_name, Address, contact, photo_path FROM fetcher WHERE Fetcher_name LIKE %s",
                (f"%{keyword}%",)
            )
            rows = cursor.fetchall()
            for row in rows:
