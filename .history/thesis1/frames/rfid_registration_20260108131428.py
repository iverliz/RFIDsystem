...
class RfidRegistration(tk.Frame):
    ...
    def delete_record(self):
        ...
        with db_connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute("DELETE FROM registrations WHERE registration_id=%s", (self.selected_reid,))
                conn.commit()
    ...
    def load_selected(self, _):
        ...
        if not data:
            return
        (self.selected_registration_id, self.selected_id, self.rfid_var.set(data[1]), self.name_var.set(data[2]),
