...
class Report(tk.Frame):
    def __init__(self, root):
        ...
        self.fetcher_table = self.create_table(
            table_frame, "FETCHERS", ["ID", "FETCHER NAME", "CONTACT", "Date"], 1, 0, colspan=2
        )
    ...
    def get_students(self):
        return self.fetch_data("SELECT ID, Student_name,grade_lvl FROM student")

    def get_teachers(self):
        return self.fetch_data("SELECT ID, Teeacher_name,Teacher_grade,  FROM teacher")

    def get_fetchers(self):
        return self.fetch_data("SELECT ID, fetcher_name, contact,  FROM fetcher")
