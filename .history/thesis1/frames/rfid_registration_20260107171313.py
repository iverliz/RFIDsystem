...
class RfidRegistration(tk.Frame):
    def __init__(self, parent, controller):
        ...
        columns = ("id", "rfid", "fetcher_name", "student_rfid", "address", "contact",
    ...
    def delete_record(self):
        ...
        with db_connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute("DELETE FROM registrations WHERE ID=%s", (self.selected_id,))
                conn.commit()
    ...
    def load_selected(self, _):
        ...
        if not data:
            return
        (self.selected_id, self.rfid_var.set(data[1]), self.name_var.set(data[2]),
    ...
    def search(self):
        ...
        with db_connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute("""
                    SELECT ID, rfid, fetcher_name, student_rfid, address, contact,
                           student_id, student_name, grade, teacher, paired_rfid
                    FROM registrations
