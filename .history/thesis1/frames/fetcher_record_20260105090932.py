...
class FetcherRecord(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # ================= HEADER (SMALLER) =================
        header = tk.Frame(self, height=65, bg="#0047AB")
        header.pack(fill="x")
        ...
        # ================= LEFT PANEL =================
        self.left_box = tk.Frame(self, width=500, height=470, bg="white")
        self.left_box.place(x=40, y=85)
        self.left_box.pack_propagate(False)

        # ================= PHOTO =================
        self.photo_frame = tk.Frame(self.left_box, width=170, height=170, bg="#E0E0E0")
        ...
        for i, (label, var) in enumerate(fields):
            tk.Label(self.left_box, text=label, bg="white").place(x=20, y=y + i * 35)
            tk.Entry(self.left_box, textvariable=var, width=30).place(x=160, y=y + i * 35)

        # ================= BUTTONS (VISIBLE) =================
        tk.Button(
            self.left_box, text="ADD",
            bg="#4CAF50", fg="white", width=12,
            command=self.add_fetcher
        ).place(x=40, y=420)

        tk.Button(
            self.left_box, text="EDIT",
            bg="#2196F3", fg="white", width=12,
            command=self.edit_fetcher
        ).place(x=190, y=420)

        tk.Button(
            self.left_box, text="DELETE",
            bg="#F44336", fg="white", width=12,
            command=self.delete_fetcher
        ).place(x=340, y=420)

        # ================= RIGHT PANEL =================
        self.right_panel = tk.Frame(self, width=550, height=440, bg="white")
        self.right_panel.place(x=600, y=100)
        ...
        tk.Entry(self.right_panel, textvariable=self.search_var, width=25).place(x=20, y=50)
        tk.Button(self.right_panel, text="Search", width=10,
                  command=self.search_fetcher).place(x=260, y=47)
        ...
        self.fetcher_table.place(x=20, y=120, width=510)
    ...
    def on_table_select(self, event):
        ...
        try:
            ...
            if f["photo_path"] and os.path.exists(f["photo_path"]):
                img = Image.open(f["photo_path"]).resize((170, 170))
                self.photo = ImageTk.PhotoImage(img)
                self.photo_label.config(image=self.photo)
                self.photo_label.image = self.photo
