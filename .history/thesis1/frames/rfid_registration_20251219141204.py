...
class RfidRegistration(tk.Frame):
    def __init__(self, root):
        ...
        tk.Label(self.fetcher_frame, text="Paired RFID (Student):", font=("Arial", 14), bg="white").place(x=20, y=240)
        self.paired_rfid_var = tk.StringVar()
    tk.Entry(self.fetcher_frame, textvariable=self.paired_rfid_var, font=("Arial", 14), width=30).place(x=180, y=240)
    ...
    try:
        conn = db_connect()
        cursor = conn.cursor()
        cursor.execute("SELECT rfid, fetcher_name, student_id, student_name, grade, teacher, paired_rfid FROM registrations")
        rows = cursor.fetchall()
        for row in rows:
            self.table.insert("", "end", values=row)
    except Exception as e:
        ...
        cursor.close()
        conn.close()
def check_access(self, rfid_tag):
    try:
        conn = db_connect()
        cursor = conn.cursor()
        cursor.execute("SELECT paired_rfid FROM registrations WHERE rfid = %s", (rfid_tag,))
        paired_rfid = cursor.fetchone()

        if paired_rfid:
            messagebox.showinfo("Access Granted", f"RFID {rfid_tag} has access, paired with {paired_rfid[0]}")
        else:
            messagebox.showerror("Access Denied", "This RFID tag is not registered or paired.")
    except Exception as e:
        messagebox.showerror("Database Error", str(e))
    finally:
        cursor.close()
...
def add_record(self):
    try:
        ...
        sql = """
        INSERT INTO registrations
        (rfid, fetcher_name, address, contact, student_id, student_name, grade, teacher, paired_rfid)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """
        values = (
            self.rfid_var.get(), self.name_var.get(), self.address_var.get(),
            self.contact_var.get(), self.student_id_var.get(), self.student_name_var.get(),
            self.grade_var.get(), self.teacher_var.get(), self.paired_rfid_var.get()  # Store paired RFID
        )
        cursor.execute(sql, values)
        conn.commit()
        messagebox.showinfo("Success", "Record added successfully")
    ...
    try:
        ...
        FROM registrations
        WHERE rfid LIKE %s OR fetcher_name LIKE %s OR student_name LIKE %s OR paired_rfid LIKE %s
        """
            value = f"%{keyword}%"
