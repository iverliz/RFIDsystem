...
from data/rfid_database.db import create_tables, insert_record, fetch_all, update_record, delete_record
...
class RfidRegistration(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # ================= TABLE =================
        columns = ("rfid", "fetcher", "address", "contact", "student_id", "student", "grade", "teacher")
    ...
    def add_record(self):
        data = (self.rfid_var.get(), self.name_var.get(), self.address_var.get(), self.contact_var.get(),
                self.student_id_var.get(), self.student_name_var.get(), self.grade_var.get(), self.teacher_var.get())
        try:
            insert_record(data)
            self.load_table()
            self.clear_fields()
    ...
    def load_table(self):
        ...
        for row in fetch_all():
            self.table.insert("", "end", values=row[1:])  # skip id column
    ...
    def edit_record(self):
        data = (self.name_var.get(), self.address_var.get(), self.contact_var.get(),
